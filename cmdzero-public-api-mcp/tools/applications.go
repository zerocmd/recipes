...
// QueryApplicationsParams defines the parameters for querying applications via HTTP QUERY.
type QueryApplicationsParams struct {
	Filter string `json:"filter,omitempty" jsonschema:"description=OData filter expression (e.g. contains(name\\, 'SIEM'))"`
	Limit  int    `json:"limit,omitempty" jsonschema:"description=Maximum number of items to return (default 10000\\, min 1\\, max 10000)"`
	Next   string `json:"next,omitempty" jsonschema:"description=Pagination cursor from a previous response"`
}

func queryApplications(ctx context.Context, args QueryApplicationsParams) (json.RawMessage, error) {
	c := mcpcmdzero.CmdZeroClientFromContext(ctx)
	if c == nil {
		return nil, fmt.Errorf("cmdzero client not configured")
	}
	...
	return c.Query(ctx, "/applications", bytes.NewReader(body))
}

var QueryApplications = mcpcmdzero.MustTool(
	"query_applications",
	"Query applications via HTTP QUERY (POST-shaped request body) for filters too long to fit in a URL. Same fields and pagination semantics as list_applications.",
	queryApplications,
	mcp.WithTitleAnnotation("Query applications"),
	mcp.WithReadOnlyHintAnnotation(true),
)
