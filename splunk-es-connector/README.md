# CZ ↔ Splunk ES Connector

A Python service that bridges Splunk Enterprise Security notables to Command Zero investigations. The connector polls Splunk for new notables, submits them to CZ for analysis, polls for verdicts, and writes results back onto the ES notable.
...
2. **Submit to CZ** — for each unsubmitted notable, POSTs a CZ investigation. Only notables whose alert type has a schema defined in `schemas.py` are submitted; notables with no mapped schema are recorded as `NO_SCHEMA` and never sent (see [Alert schemas](#alert-schemas) — this is an intentional filter). On the first submission of a given alert type, the connector includes an `alertSchema` so CZ knows which fields contain observables; CZ caches this schema for subsequent submissions.
3. **Poll CZ** — fetches the current status of all open investigations. When an investigation carries a non-empty verdict at any post-automation status (`pending-review`, `in-progress`, `on-hold`, or `completed`), the record is marked ready. Investigations that never produce a verdict within one day are given up on and marked failed.
4. **Write back** — optionally posts the verdict as a comment on the ES notable via `notable_update` (requires ES; can be disabled with `SPLUNK_ES_WRITEBACK=false`), then sends a structured enrichment event to a Splunk HEC index (`cz_enrichment`) for dashboarding. When the comment step is enabled, it runs first and its success is persisted before the HEC step, so a HEC failure that triggers a retry never re-posts the comment.
...
2. Click **New Token**.
3. Set a meaningful name (e.g. `cz-connector`) and an appropriate expiry.
...
The HEC path writes structured enrichment events to a `cz_enrichment` index.
...
2. **Create a token:** New Token → name it (e.g. `cz-enrichment`) → set the default index to `cz_enrichment`.
3. The token value is the value for `SPLUNK_HEC_TOKEN`.
4. **Create the `cz_enrichment` index** (Splunk will reject HEC events to a non-existent index):

   ```bash
   curl -sk -u admin:'<password>' https://<splunk-host>:8089/services/data/indexes \
     -d name=cz_enrichment
   ```

   Or: Settings → Indexes → New Index → name `cz_enrichment`.

### 5. Configure credentials

```bash
cp .env.example .env
# Edit .env — at minimum set CZ_ORG_ID, CZ_BEARER_TOKEN, SPLUNK_REST_URL, SPLUNK_SVC_TOKEN
```
...
The connector performs a preflight check on startup, verifying connectivity and authentication for both Splunk and CZ before entering the main loop. If any required value is missing or authentication fails, it exits with a clear error message.
...
|---|---|---|---|
| `CZ_ORG_ID` | yes | — | Your CZ organization UUID |
| `CZ_BEARER_TOKEN` | yes | — | CZ API bearer token |
| `CZ_API_BASE_URL` | no | `https://api.cmdzero.io/public/v1` | CZ API base URL |
...
| `ENRICHMENT_INDEX` | no | `cz_enrichment` | Splunk HEC index for enrichment events |
...
| `CZ_POLL_INTERVAL` | no | `300` | Base seconds between CZ verdict polls (exponential backoff applied per record) |
| `CZ_POLL_MAX_BACKOFF` | no | `1800` | Per-record backoff ceiling (seconds) |
| `CZ_SUBMIT_DELAY` | no | `2.0` | Pause (seconds) between consecutive CZ submissions to avoid burst rate-limiting |
| `ALERT_TYPE_PREFIX` | no | `SplunkES` | Prefix applied to alert types sent to CZ (e.g. `SplunkES:Access - Brute Force...`) |
| `AUTO_DISPOSITION` | no | `false` | **Not yet implemented.** Reserved flag intended to automatically set the ES notable disposition from the CZ verdict. Requires a live Splunk ES instance to build and test against — see the implementation note in `config.py`. Setting it has no effect today. |
...
**Explicit schema** — a list of TypeAnnotation dicts that tell CZ which fields contain observables. The connector sends this on the first submission of the alert type; CZ caches it for subsequent submissions.
...
**Auto-schema (`None`)** — the alert type is allowed but no schema is provided. CZ will infer observables from the alert data automatically using auto-schema.

```python
SCHEMAS["SplunkES:My New Rule - Rule"] = None
```

**Type values must be valid CZ lead types** — invalid types cause a `400 Unknown type` error at submission time. Common types:
...
Alert types not present in `SCHEMAS` are stored locally as `NO_SCHEMA` (a terminal state) and never submitted to CZ. This is intentional — the connector only forwards correlation searches you have explicitly opted in, keeping the integration deliberate during rollout.
...
cz.py          — CZ API client (submit, poll, 429 retry)
...
NO_SCHEMA  (terminal — recorded at discovery for alert types with no mapped schema;
            never submitted to CZ; see "Alert schemas")
```
...
`VERDICT_READY` means CZ has finished its automated analysis and a verdict is ready to write back to Splunk. This is triggered by a non-empty verdict at any post-automation status (`pending-review`, `in-progress`, `on-hold`, `completed`) — not just `pending-review` — so a verdict isn't lost if an analyst has already advanced the case. The actual CZ status is passed through to the `cz_enrichment` HEC index as `cz_status`.
...
The CZ client retries `429 Too Many Requests` responses up to 3 times, honoring the `Retry-After` header (defaulting to 60 seconds if the header is absent). The `CZ_SUBMIT_DELAY` setting (default 2 seconds) adds a pause between consecutive POSTs to avoid hitting burst limits in the first place.
